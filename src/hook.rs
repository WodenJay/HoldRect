...
pub fn start_hook_listener(
    ...
    *MODIFIER_CODES
        .write()
        .expect("MODIFIER_CODES RwLock poisoned") = modifier_codes;
    ...
    std::thread::spawn(move || unsafe {
        ...
        let _mouse = SetWindowsHookExW(WH_MOUSE_LL, Some(mouse_hook_proc), HINSTANCE::default(), 0)
...
unsafe extern "system" fn keyboard_hook_proc(
    ...
    if n_code >= 0 {
        ...
        let is_key_down =
            w_param == WPARAM(WM_KEYDOWN as usize) || w_param == WPARAM(WM_SYSKEYDOWN as usize);
        ...
        let modifier_held = SHOULD_SUPPRESS.load(Ordering::Relaxed);
        if let Some(event) = decide_keyboard(
            kb.vkCode,
            is_key_down,
            &MODIFIER_CODES
                .read()
                .expect("MODIFIER_CODES RwLock poisoned"),
            modifier_held,
        ) {
...
unsafe extern "system" fn mouse_hook_proc(
    ...
    if n_code >= 0 {
        ...
        let (event, should_suppress) = decide_mouse(
            msg,
            pt,
            suppress,
            drag,
        ...
        if should_suppress {
            match msg {
                WM_LBUTTONDOWN => {
                    DRAG_IN_PROGRESS.store(true, Ordering::Relaxed);
                }
                WM_LBUTTONUP => {
                    DRAG_IN_PROGRESS.store(false, Ordering::Relaxed);
                }
                _ => {}
            }
        }
...
pub(crate) fn decide_keyboard(
    ...
    if modifier_codes.contains(&vk_code) {
        return Some(InputEvent::ModifierChanged {
            pressed: is_key_down,
        });
    }
    if is_key_down {
...
mod tests {
    ...
    fn mouse_leftdown_no_suppress_passes_through() {
        let (event, suppress) =
            decide_mouse(WM_LBUTTONDOWN, (100, 200), false, false, false, false, 0);
    ...
    fn mouse_leftdown_suppress_ctrl_held_is_suppressed() {
        let (event, suppress) =
            decide_mouse(WM_LBUTTONDOWN, (100, 200), true, false, true, false, 0);
    ...
    fn mouse_leftdown_suppress_but_no_ctrl_passes_through() {
        let (event, suppress) =
            decide_mouse(WM_LBUTTONDOWN, (100, 200), true, false, false, false, 0);
        assert_eq!(event, None);
        assert!(!suppress);
    }
    ...
    fn mouse_move_drag_in_progress_tracks_but_passes_through() {
        let (event, suppress) =
            decide_mouse(WM_MOUSEMOVE, (300, 400), false, true, false, false, 0);
        assert_eq!(event, Some(InputEvent::MouseMove { x: 300, y: 400 }));
        assert!(
            !suppress,
            "MouseMove must pass through during drag so cursor stays responsive"
        );
    }

    #[test]
    fn mouse_move_no_drag_passes_through() {
        let (event, suppress) = decide_mouse(WM_MOUSEMOVE, (300, 400), true, false, true, false, 0);
        assert_eq!(event, None);
        assert!(!suppress);
    }
    ...
    fn mouse_leftup_drag_in_progress_is_suppressed() {
        let (event, suppress) =
            decide_mouse(WM_LBUTTONUP, (500, 600), false, true, false, false, 0);
    ...
    fn mouse_leftup_no_drag_passes_through() {
        let (event, suppress) = decide_mouse(WM_LBUTTONUP, (500, 600), true, false, true, false, 0);
        assert_eq!(event, None);
        assert!(!suppress);
    }
    ...
    fn mouse_rightdown_suppress_passes_through() {
        let (event, suppress) =
            decide_mouse(WM_RBUTTONDOWN, (100, 200), true, false, true, false, 0);
        assert_eq!(event, None);
        assert!(!suppress);
    }
    ...
    fn mouse_rightdown_drag_in_progress_passes_through() {
        let (event, suppress) =
            decide_mouse(WM_RBUTTONDOWN, (100, 200), false, true, false, false, 0);
        assert_eq!(event, None);
        assert!(!suppress);
    }
    ...
    fn full_alt_drag_sequence() {
        ...
        assert!(
            suppress,
            "LButtonDown should be suppressed when Alt+suppress active"
        );
    ...
    fn alt_held_mouse_moves_pass_through_before_drag() {
        ...
        assert_eq!(
            event, None,
            "MouseMove must NOT generate event before drag starts"
        );
    ...
    fn alt_released_before_mouse_up_drag_still_suppressed() {
        ...
        assert!(
            suppress,
            "LButtonUp must be suppressed even if Alt released during drag"
        );
    }
    ...
    fn alt_released_mouse_move_during_drag_passes_through() {
        ...
        assert!(
            !suppress,
            "MouseMove must pass through during drag even if Ctrl released"
        );
    }
    ...
    fn alt_key_with_alt_config_detected() {
        ...
        assert_eq!(
            decide_keyboard(VK_LMENU.0 as u32, true, &codes, false),
    ...
    fn ctrl_key_with_ctrl_config_detected() {
        ...
        assert_eq!(
            decide_keyboard(VK_LCONTROL.0 as u32, true, &codes, false),
            Some(InputEvent::ModifierChanged { pressed: true })
        );
    }
    ...
    fn shift_key_with_shift_config_detected() {
        ...
        assert_eq!(
            decide_keyboard(VK_LSHIFT.0 as u32, true, &codes, false),
            Some(InputEvent::ModifierChanged { pressed: true })
        );
    }
    ...
    fn win_key_with_win_config_detected() {
        ...
        assert_eq!(
            decide_keyboard(VK_LWIN.0 as u32, true, &codes, false),
            Some(InputEvent::ModifierChanged { pressed: true })
        );
    }
    ...
    fn decide_keyboard_zero_vk_code_in_codes() {
        ...
        assert_eq!(
            decide_keyboard(0, true, &codes, false),
            Some(InputEvent::ModifierChanged { pressed: true })
        );
    }
    ...
    fn mouse_drag_in_progress_with_suppress_flag_true_move_still_passes_through() {
        ...
        let (event, suppress) = decide_mouse(WM_MOUSEMOVE, (300, 400), true, true, true, false, 0);
        assert_eq!(event, Some(InputEvent::MouseMove { x: 300, y: 400 }));
        assert!(
            !suppress,
            "MouseMove must pass through during drag regardless of suppress state"
        );
    }
    ...
    fn mouse_lbuttondown_during_drag_is_noop() {
        ...
        let (event, suppress) =
            decide_mouse(WM_LBUTTONDOWN, (100, 200), true, true, true, false, 0);
        assert_eq!(
            event, None,
            "LButtonDown during active drag should be ignored"
        );
        assert!(
            !suppress,
            "LButtonDown during active drag should not suppress"
        );
    }
    ...
    fn mouse_mbuttonup_during_drag_is_noop() {
        let (event, suppress) =
            decide_mouse(WM_MBUTTONUP, (100, 200), false, true, false, false, 0);
        assert_eq!(event, None);
        assert!(!suppress);
    }
    ...
    fn mouse_unknown_message_during_drag_is_noop() {
        ...
        let (event, suppress) = decide_mouse(0x020B, (100, 200), false, true, false, false, 0);
        assert_eq!(event, None);
        assert!(!suppress);
    }
    ...
    fn mouse_unknown_message_outside_drag_suppress_modifier_held_is_noop() {
        ...
        let (event, suppress) = decide_mouse(0x020B, (100, 200), true, false, true, false, 0);
        assert_eq!(event, None);
        assert!(!suppress);
    }
    ...
    fn mouse_move_suppress_true_modifier_held_true_no_drag_is_noop() {
        ...
        let (event, suppress) = decide_mouse(WM_MOUSEMOVE, (100, 200), true, false, true, false, 0);
        assert_eq!(event, None);
        assert!(!suppress);
    }
    ...
    fn drag_with_negative_coordinates() {
        ...
        assert_eq!(
            event,
            Some(InputEvent::MouseButtonDown { x: -100, y: -200 })
        );
        assert!(suppress);
    }
    ...
    fn mouse_scroll_magnifier_not_active_passes_through() {
        ...
        let (event, suppress) =
            decide_mouse(0x020A, (100, 200), true, false, true, false, mouse_data);
        assert_eq!(event, None);
        assert!(!suppress);
    }
    ...
    fn mouse_scroll_modifier_not_held_magnifier_active_still_works() {
        ...
        let mouse_data: isize = (120i32 << 16) as isize;
        let (event, suppress) =
            decide_mouse(0x020A, (100, 200), false, false, false, true, mouse_data);
        assert_eq!(event, Some(InputEvent::ScrollUp));
        assert!(suppress);
    }
    ...
    fn mouse_scroll_during_drag_magnifier_active_still_works() {
        ...
        let mouse_data: isize = (120i32 << 16) as isize;
        let (event, suppress) =
            decide_mouse(0x020A, (100, 200), true, true, true, true, mouse_data);
        assert_eq!(event, Some(InputEvent::ScrollUp));
        assert!(suppress);
    }
