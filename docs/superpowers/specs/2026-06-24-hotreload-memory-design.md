...
**关键约束: baseline必须用未优化的release二进制测量, 在任何Cargo.toml profile改动之前。**

步骤:
1. 用当前Cargo.toml(无lto/strip/panic等优化)编译 `cargo build --release`
2. 运行 `./target/release/holdrect.exe --mem-report`
3. 记录 Working Set Size 和 Pagefile Usage 作为 **BASELINE**
4. 将BASELINE数字写入 commit message 或测试日志, 确保可追溯

`--mem-report` 在程序启动后立即采样并退出(不启动hook/overlay/tray), 测量的是进程基础内存占用。如果BASELINE已 < 3MB, 编译优化仍执行(免费收益), 但运行时优化(Task 8)跳过。
...
1. **内存优化 — 实现--mem-report** (代码改动, 不改Cargo.toml)
2. **内存优化 — 测量BASELINE** (用未优化的cargo build --release, 记录数字, 这是所有优化的基准线)
3. **内存优化 — 编译参数** (Cargo.toml优化, 再测一次, 对比BASELINE)
4. **热加载 — hook.rs** (RwLock改造 + update函数)
5. **热加载 — config.rs** (watch_config_dir + PartialEq)
6. **热加载 — overlay.rs** (App持有config, about_to_wait poll, PopupManager cascade)
7. **热加载 — main.rs** (启动watcher线程, channel连接)
8. **热加载 — 测试** (TDD: 先写测试再实现)
9. **内存优化 — 运行时** (仅当BASELINE > 3MB时执行)
10. **最终验证** (全部测试 + 内存测量对比BASELINE)
